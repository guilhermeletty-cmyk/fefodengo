...
def gerar_excel(sheets: dict) -> bytes:
    ...
    Tenta xlsxwriter primeiro (mais compatível), depois openpyxl.
    Se nenhum engine estiver disponível, retorna um ZIP com CSVs.
    """
    engine = _excel_engine()

    # ── Fallback: ZIP com CSVs quando nenhum engine Excel está disponível ──
    if engine == 'none':
        import zipfile
        buf = io.BytesIO()
    ...
    # ── Cores por palavra-chave de status ──────────────────────────────────
    STATUS_COLORS = {
    ...
    DEFAULT_COLOR = 'FFE0B2'   # laranja suave
    HDR_BG        = '1a1a2e'   # azul escuro
    ...
    if engine == 'xlsxwriter':
        with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
            ...
            # Cache de formatos de cor de linha
            fmt_cache = {}
            def _fmt(color):
            ...
            for sname, (df, col_status) in sheets.items():
                if not isinstance(df, pd.DataFrame) or df.empty:
                    continue
                sname31 = sname[:31]
                df.to_excel(writer, index=False, sheet_name=sname31)
                ws = writer.sheets[sname31]
    ...
    # ── engine: openpyxl (fallback) ────────────────────────────────────────
        ...
        with pd.ExcelWriter(buf, engine='openpyxl') as writer:
            for sname, (df, col_status) in sheets.items():
                ...
                if col_status and col_status in df.columns:
                    idx = df.columns.get_loc(col_status) + 1
