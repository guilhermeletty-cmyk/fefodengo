...
def _limpar_valor(val):
    ...
    Evita NaN, Inf, NaT, Timestamp e tipos numpy não serializáveis.
    """
    import math
    ...
    if isinstance(val, float):
        if math.isnan(val) or math.isinf(val):
            return ''
    # Converter Timestamp/datetime para string legível
...
def _sanitizar_nome_aba(nome: str) -> str:
    """
    Limpa o nome da aba Excel:
    Remove/substitui caracteres proibidos e limita a 31 chars.
    Proibidos pelo Excel: / ? * [ ] : e travessao unicode
    """
    import re
    nome = str(nome).replace('\u2013', '_').replace('\u2014', '_')
    ...
    if engine == 'none':
        ...
        with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
            for sname, (df, _) in sheets.items():
                ...
                zf.writestr(f"{_sanitizar_nome_aba(sname)}.csv", csv_bytes)
        return buf.getvalue()
    ...
    if engine == 'xlsxwriter':
        ...
        with pd.ExcelWriter(buf, engine='xlsxwriter',
            ...
            for sname, (df, col_status) in sheets.items():
                ...
                sname31    = _sanitizar_nome_aba(sname)
    ...
    # ── engine: openpyxl (fallback) ───────────────────────────────────────
        ...
        with pd.ExcelWriter(buf, engine='openpyxl') as writer:
            for sname, (df, col_status) in sheets.items():
                ...
                sname_safe = _sanitizar_nome_aba(sname)
                df_clean.to_excel(writer, index=False, sheet_name=sname_safe)
