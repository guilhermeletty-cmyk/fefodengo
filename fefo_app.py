...
import pandas as pd
import numpy as np
import io, re, math, unicodedata, zipfile
...
# ── helpers Excel nativos (zero dependências externas) ─────────────────────────

def _val_safe(val):
    """Converte qualquer valor para tipo seguro para XLSX nativo."""
...
def gerar_excel(sheets: dict) -> bytes:
    """
    Gera XLSX 100% com zipfile + XML puro.
    Zero dependencia de openpyxl ou xlsxwriter.
    Funciona em qualquer Python 3.6+ e qualquer ambiente cloud.
    """
    STATUS_COLORS = {
        'INCORRETO':'FFCCCC','QUEBRA':'FFCCCC','DEVOLVER':'FFCCCC','RUPTURA':'FFCCCC',
        'BUSCAR':'CCE5FF','MOVER':'CCE5FF','REABASTEC':'CCE5FF',
        'CORRETO':'D4EDDA','OK':'D4EDDA','SOBRA':'D4EDDA',
    }
    DEFAULT_ROW = 'FFE0B2'
    HDR_BG      = '1A1A2E'
    ...
    # Pre-processar
    clean = {}
    for sname, (df, col_st) in sheets.items():
        if not isinstance(df, pd.DataFrame) or df.empty: continue
        df_c = df.copy()
        for c in df_c.columns: df_c[c] = df_c[c].apply(_val_safe)
        clean[_sname(sname)] = (df_c, col_st)
    if not clean:
        clean['Sem dados'] = (pd.DataFrame({'Info':['Sem dados']}), None)

    # Shared strings
    all_str, str_idx = [], {}
    def sid(s):
        s = str(s)
        if s not in str_idx: str_idx[s] = len(all_str); all_str.append(s)
        return str_idx[s]

    # Cores unicas
    _c_order = [HDR_BG, DEFAULT_ROW] + list(STATUS_COLORS.values())
    seen_c, uniq_c = set(), []
    for c in _c_order:
        if c not in seen_c: seen_c.add(c); uniq_c.append(c)
    cidx = {c:i for i,c in enumerate(uniq_c)}

    fills_xml  = '<fills count="{}">'.format(len(uniq_c)+2)
    fills_xml += '<fill><patternFill patternType="none"/></fill>'
    fills_xml += '<fill><patternFill patternType="gray125"/></fill>'
    for c in uniq_c:
        fills_xml += '<fill><patternFill patternType="solid"><fgColor rgb="FF{}"/></patternFill></fill>'.format(c)
    fills_xml += '</fills>'

    hdr_fi  = cidx[HDR_BG] + 2
    xf_list = [
        '<xf numFmtId="0" fontId="0" fillId="0" borderId="0"/>',
        '<xf numFmtId="0" fontId="1" fillId="{}" borderId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment horizontal="center"/></xf>'.format(hdr_fi),
    ] + ['<xf numFmtId="0" fontId="0" fillId="{}" borderId="0" applyFill="1"/>'.format(cidx[c]+2) for c in uniq_c]
    xf_color = {c: 2+cidx[c] for c in uniq_c}
    ...
    styles_xml = (
        ...
        '<cellXfs count="{}">{}</cellXfs>'.format(len(xf_list), ''.join(xf_list)) +
        '</styleSheet>'
    )

    # Worksheets XML
    sheet_xmls = {}
    for sn, (df_c, col_st) in clean.items():
        cols   = list(df_c.columns)
        st_idx = cols.index(col_st) if (col_st and col_st in cols) else None
        rows   = ''
        cells  = ''.join('<c r="{}{}" t="s" s="1"><v>{}</v></c>'.format(_col_letter(ci), 1, sid(c)) for ci,c in enumerate(cols))
        rows  += '<row r="1">{}</row>'.format(cells)
        for ri, rv in enumerate(df_c.itertuples(index=False), start=2):
            color = _row_color(rv[st_idx]) if st_idx is not None else DEFAULT_ROW
            xs    = xf_color.get(color, 2)
            cells = ''
            for ci, v in enumerate(rv):
                ref = '{}{}'.format(_col_letter(ci), ri)
                v   = _val_safe(v)
                if v == '':
                    cells += '<c r="{}" s="{}"/>'.format(ref, xs)
                elif isinstance(v, (int, float)):
                    cells += '<c r="{}" s="{}"><v>{}</v></c>'.format(ref, xs, v)
                else:
                    cells += '<c r="{}" t="s" s="{}"><v>{}</v></c>'.format(ref, xs, sid(str(v)))
            rows += '<row r="{}">{}</row>'.format(ri, cells)
        sheet_xmls[sn] = (
            '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            '<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">'
            '<sheetData>{}</sheetData></worksheet>'.format(rows)
        )
    ...
    ct = (
        ...
        + ''.join('<Override PartName="/xl/worksheets/sheet{}.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/>'.format(i+1) for i in range(n))
        + '</Types>'
    )
    ...
    wb_rels = (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">'
        + ''.join('<Relationship Id="rId{}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet{}.xml"/>'.format(i+1,i+1) for i in range(n))
        + '<Relationship Id="rId{}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/sharedStrings" Target="sharedStrings.xml"/>'.format(n+1)
        + '<Relationship Id="rId{}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/>'.format(n+2)
        + '</Relationships>'
    )
    ...
    wb_xml = (
        ...
        '<sheets>'
        + ''.join('<sheet name="{}" sheetId="{}" r:id="rId{}"/>'.format(_xml_esc(nm), i+1, i+1) for i,nm in enumerate(names))
        + '</sheets></workbook>'
    )

    ss_items = ''.join('<si><t xml:space="preserve">{}</t></si>'.format(_xml_esc(s)) for s in all_str)
    ss = (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        '<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" '
        'count="{0}" uniqueCount="{0}">{1}</sst>'.format(len(all_str), ss_items)
    )
    ...
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        for i, (sn, xml) in enumerate(sheet_xmls.items()):
            zf.writestr('xl/worksheets/sheet{}.xml'.format(i+1), xml)
    return buf.getvalue()
